...
do
    if [ $applications_install_sh = "${PWD}/applications-install-sh/${release_like_name}-install" ]; then
        echo $applications_install_sh
        [[ ! "$(ls -A $applications_install_sh)" ]] && echo "no install bash shell files." && exit
        for application_sh in "$applications_install_sh"/*.sh
        do
        echo $application_sh
        [[ -f $application_sh ]] && sudo chmod +x "$application_sh" && . "$application_sh"
        done
    fi
done
